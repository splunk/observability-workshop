var relearn_search_index=[{content:` 5 minutes How to retrieve the IP address of the AWS/EC2 instance assigned to you. Connect to your instance using SSH, Putty1 or your web browser. Verify your connection to your AWS/EC2 cloud instance. 1. AWS/EC2 IP Address In preparation for the workshop, Splunk has prepared an Ubuntu Linux instance in AWS/EC2.
...
`,description:"Deploy the Online Boutique application into Kubernetes (K3s) and generate some artificial traffic using Locust.",tags:null,title:"1. Deploy the Online Boutique",uri:"/apm/online-boutique/index.html"},{content:`1. New Kubernetes Navigator 2.0 UI As we are in the process of switching to the new generation of the Kubernetes Navigator, please check if you are already on the new Kubernetes navigator.
...
Note If you actually see three services for Kubernetes including one that is named K8s clusters, you need to turn off Precognition in the Superpowers view. To do this, please change the Url in your browser to match the following: https://app.[REALM].signalfx.com/#/superpowers
...
2. Connect to EC2 instance You will be able to connect to the workshop instance by using SSH from your Mac, Linux or Windows device.
...
Note Your workshop instance has been pre-configured with the correct ACCESS_TOKEN and REALM for this workshop. There is no need for you to configure these.
...
helm delete splunk-otel-collector -n splunk `,description:"",tags:null,title:"Deploying the OpenTelemetry Collector in Kubernetes using a NameSpace",uri:"/tko/session-5/deploy-otel/index.html"},{content:`Aim The aim of this module is for you to configure your personal profile which controls how you will be notified by Splunk On-Call whenever you get paged.
...
`,description:"",tags:null,title:"User Profile",uri:"/oncall/getting_started/index.html"},{content:`1. Introduction The OpenTelemetry Collector is the core component of instrumenting infrastructure and applications. Its role is to collect and send:
...
2. Configure environment variables If you have already completed the Splunk IM workshop you can take advantage of the existing environment variables. Otherwise, create the ACCESS_TOKEN and REALM environment variables to use in the proceeding OpenTelemetry Collector install command. For instance, if your realm is us1, you would type export REALM=us1 and for eu0 type export REALM=eu0 etc.
...
`,description:"",tags:null,title:"Installing the OpenTelemetry Collector",uri:"/other/pet-clinic/docs/imt/index.html"},{content:`Overview of the RUM Workshop The aim of this Splunk Real User Monitoring (RUM) workshop is to let you:
...
`,description:"",tags:null,title:"1. Overview",uri:"/rum/1-overview/index.html"},{content:`Aim The aim of this module is for you to complete the first step of Team configuration by adding users to your Team.
...
`,description:"",tags:null,title:"Teams",uri:"/oncall/getting_started/team/index.html"},{content:`Aim The aim of this module is for you to get more familiar with the Timeline Tab and the filtering features.
...
`,description:"",tags:null,title:"Incident Lifecycle",uri:"/oncall/incident_lifecycle/index.html"},{content:`Aim The aim of this module is for you to place yourself ‘On-Call’ then generate an Incident using the supplied EC2 Instance so you can then work through the lifecycle of an Incident.
...
`,description:"",tags:null,title:"Teams",uri:"/imt/servicebureau/teams/index.html"},{content:`The goal is to walk through the basic steps to configure the following components of the Splunk Observability platform:
...
JDK 17 installed Maven Port 8080 open inbound/outbound `,description:"A workshop using Zero Configuration Auto-Instrumentation for Java",tags:null,title:"PetClinic Java Workshop",uri:"/other/pet-clinic/index.html"},{content:`During this technical Splunk Observability Cloud Infrastructure Monitoring and APM Workshop you will build out an environment based on a lightweight Kubernetes1 cluster.
...
Other WorkshopsPetClinic Java WorkshopA workshop using Zero Configuration Auto-Instrumentation for Java
`,description:"Learn how to build observability solutions with Splunk",tags:null,title:"Splunk Observability Workshops",uri:"/index.html"},{content:`This Lab walks your through using the Chrome Selenium IDE extension to create a synthetic transaction against a Splunk demo instance and creating a Splunk Synthetic Monitoring Real Browser Check (RBC). In addition you also get to learn other Splunk Synthetic Monitoring checks like REST API checks and Uptime Checks.
...
Selenium IDE
`,description:"",tags:null,title:"Real Browser Checks",uri:"/synthetics/real-browser-checks/index.html"},{content:` 5 minutes 1. Generate traffic The Online Boutique deployment contains a container running Locust that we can use to generate load traffic against the website to generate metrics, traces and spans.
...
Lynx Command lynx http://localhost:55679/debug/tracez `,description:"",tags:null,title:"1.1 Generate load using Locust",uri:"/apm/online-boutique/locust/index.html"},{content:`Aim A rotation is a recurring schedule, that consists of one or more shifts, with members who rotate through a shift.
...
`,description:"",tags:null,title:"Deploying the OpenTelemetry Collector in Kubernetes",uri:"/imt/gdi/index.html"},{content:`1. Cluster vs Workload View The Kubernetes Navigator offers you two separate use cases to view your Kubernetes data.
The K8s workloads is focusing on providing information in regards to workloads a.k.a. your deployments. The K8s nodes is focusing on providing insight into the performance of clusters, Nodes, Pods \u0026 Containers. You initially select either view depending on your need. (You can switch between the view on the fly if required) The most common one we will use in this workshop is the workload view and we will focus on that specifically.
1.1 Finding your K8s cluster name Your first task is to identify and find your own cluster. The cluster will be named after your EC2 instance name: ws-5-X-k3s-cluster where X is the number of the EC2 instance assigned to you.
...
2. Workloads \u0026 Workload Details Pane Go to the Infrastructure menu item in the Observability UI and select Kubernetes. Go to the Infrastructure page in the Observability UI and select Kubernetes, this will offer you a set of Kubernetes services, one of them being the K8s workloads pane
...
Now let’s change the list view to a heat map view by selecting either the Heat map icon or List icon in the upper-right corner of the screen: heat-map-toggle (as marked with a purple line)
...
Workshop Question What happened to the Heatmap?
...
2.2 The Workload Overview pane The workload overview gives you a quick insight of the status of your deployment. You can see at once if the pods of your deployments are Pending, Running, Failed, Succeeded or in an unknown state.
Running: Pods are deployed and in a running state Pending: Waiting to be deployed Succeeded: Pod has been deployed and completed its job and is finished Failed: Containers in the pod have run and returned some kind of error Unknown: Kubernetes isn’t reporting any of the known states. (This may be during start or stopping of pods, for example). You can expand the Workload name by hovering your mouse on it, in case the name is longer than the chart allows.
...
This will add the selected workload to your filters. Try this for the splunk-otel-collector-k8s-cluster-receiver workload. It should give you a single rectangle in the \`splunk’ namespace. Confirm that you have the right filter by hovering over the rectangle, and if it is indeed the splunk-otel-collector-k8s-cluster-receiver workload, double click on it.
...
3. Pivot Sidebar Later in the workshop, you will deploy an Apache server into your cluster which will cause a pivot bar to appear. As soon as any metric that is used by a Navigator, is flowing into your Observability org, the system will add that service to the pivot bar.
...
`,description:"",tags:null,title:"Tour of the Kubernetes Navigator v2",uri:"/tko/session-5/check-new-navigator-short/index.html"},{content:` Find the Web address of your workshop hosts Online Boutique Generate traffic by shopping for bargains on your workshop hosted Online Boutique web shop. 1. URL of RUM enabled Online Boutique As discussed in the previous section we are going to use an Online Boutique running on a RUM host. If you’re participating in a RUM only workshop, after you have received the RUM instance URL, you can continue to Section 4: Using the Online Boutique to generate load on your system as the system your going to use is already prepared.
...
`,description:"",tags:null,title:"2. Showcase of RUM with the Online Boutique",uri:"/rum/2-showcase/index.html"},{content:`1. Spring PetClinic Application First thing we need to setup APM is… well, an application. For this exercise, we will use the Spring PetClinic application. This is a very popular sample java application built with Spring framework (Springboot).
...
cd spring-petclinic Start a MySQL database for PetClinic to use:
...
./mvnw package -Dmaven.test.skip=true Information This will take a few minutes the first time you run, maven will download a lot of dependencies before it actually compiles the app. Future executions will be a lot shorter.
...
Once your validation is complete you can stop the application by pressing Ctrl-c .
2. Adding Resource Attributes to Spans Resource attributes can be added to every reported span. For example version=0.314. A comma separated list of resource attributes can also be defined e.g. key1=val1,key2=val2.
...
`,description:"",tags:null,title:"2.1 Service Map",uri:"/apm/using-splunk-apm/service_map/index.html"},{content:`1. Acknowledge Use your Splunk On-Call App on your phone to acknowledge the Incident by clicking on the push notification …
...
`,description:"",tags:null,title:"Manage Incidents",uri:"/oncall/incident_lifecycle/manage_incidents/index.html"},{content:`1. Saving a chart To start saving your chart, lets give it a name and description. Click the name of the chart Copy of Latency Histogram and rename it to “Active Latency”.
...
`,description:"",tags:null,title:"Controlling Usage",uri:"/imt/servicebureau/tokens/index.html"},{content:`Set Environment Variables Environment Variables:
...
Exit nano
CTRL-X
Build and Deploy Application Let’s get started by building and deploying our Application, the Buttercup Instrument Shop. Run the commands below to begin and start reading ahead as your traces are coming up !
cd javashop-otel ./BuildAndDeploy.sh Users and Workflows As we go through this workshop we will be switching roles from SRE to Developer. First we will start with alert responders or SREs who will identify an issue in Splunk Observability UI. Next, we will jump to a Developer Role to see how a Developer will debug and repair/fix a software problem using trace data provided by our SRE.
Of course, we are not requiring 2 people for this workshop as each participant will play both roles.
Today we will learn Today we will learn the type of data ( In the form of a trace ) that an SRE or alert responder would send to a developer to then repair/fix software. We will do this with both Auto-Instrumentation data and Custom attributes, via Manual Instrumentation data.
...
Let’s define a few terms for those new to APM / Software Development or Java What’s a Function in Java ? A Function in most languages including Java, is a logical chunk of code when executed that solves a repeatable task. This is basically what our customers’ Dev teams spend their time building and where software issues will most commonly be. What’s a Method in Java ? See What’s a function -\u003e Function and method are synonymous. What’s an Exception in Java ? An Exceptional error condition that indicates abnormal or unhandled condition, that interrupts program execution abnormally. View Service Map Please note it may take 3-4 minutes for traces to show up and you will see full map “form” as traces are coming in, so you may have to refresh the page a few times each time we Build and Deploy.
...
Click on shop service Click Traces ( right side ) Sort by Duration Select the longest duration trace ( or one of the obvious much longer ones )
...
Now let’s play the role of the developer As a developer we must debug the function products:ProductResource.getAllProducts to find the problem.
Debugging 101, the Line by Line method Without anything to go on other than “BAD FUNCTION”, a Developer must then look at code visually line by line to find and fix the problem. To make this worse, Functions, call Functions and it can get very messy in bad code scenarios.
We will do the visual inspection method next.
Using Nano:
nano products/src/main/java/com/shabushabu/javashop/products/resources/ProductResource.java Search in Nano CTRL-W
Enter in: getAllProducts
@GET public Response getAllProducts(@DefaultValue("California") @QueryParam("location") String location) { // STEP X: All we know right now is somewhere in this function, latency was introduced. myCoolFunction1(location); myCoolFunction2(location); myCoolFunction10(location); myCoolFunction13(location); myCoolFunction5(location); myCoolFunction6(location); We can see here in getAllProducts, the first call is to myCoolFunction1(), so as may have guessed our next step is to go look at myCoolFunction1()
...
Custom Attribution To take a deeper look at this issue and make this much easier to debug we will implement Custom Attributes via Opentelemetry Manual Instrumentation.
...
Run Manual Instrumentation Tool cd javashop-otel directory ./AutomateManualInstrumentation.sh Rebuild and Deploy Application ./BuildAndDeploy.sh Now that we have rebuilt and deployed our application, traffic is being sent once again.
...
In our example the “Location” tag was created due our handy Annotator, that did the OpenTelemetry Annotations for us.
Using nano:
nano products/src/main/java/com/shabushabu/javashop/products/resources/ProductResource.java Search in Nano CTRL-W
Enter in: 999
We can see here, someone wrote some very bad code in the form of a long Sleep !
if (999==myInt) Thread.sleep( sleepy.nextInt(5000 - 3000) + 3000); ) How did we get here ? How did the 999 end up in the trace as a Custom Attribute ? Take a look at the function signature
...
Let’s fix our code.
Change this:
private void myCoolFunction234234234(@SpanAttribute("myInt") int myInt) { // Generate a FAST sleep of 0 time ! Random sleepy = new Random(); try{ if (999==myInt) Thread.sleep( sleepy.nextInt(5000 - 3000) + 3000); } catch (Exception e){ To this:
private void myCoolFunction234234234(@SpanAttribute("myInt") int myInt) { // Generate a FAST sleep of 0 time ! Random sleepy = new Random(); try{ // if (999==myInt) // Thread.sleep( // sleepy.nextInt(5000 - 3000) // + 3000); } catch (Exception e){ Using nano
place comments before every line in myCoolFunction234234234
// if (999==myInt) // Thread.sleep( // sleepy.nextInt(5000 - 3000) // + 3000); Write out changes in nano
CTRL-O
CTRL-X to exit.
...
Let’s go see if our manual instrumentation uncovered any other issues we did not see before So you may be asking yourself, “How does manual instrumentation alone show us “more problems” than before latency is latency is it ?
...
Click on shop service Click Traces ( right side ) Select “Errors Only” Note: we haven’t changed the code at all by adding annotations.
...
Let’s play Developer once again and fix our issue We already know exactly what file to look in and what method to look at as it is called out in the trace.
Edit the file
Using nano
nano shop/src/main/java/com/shabushabu/javashop/shop/model/Instrument.java Search in Nano CTRL-W
Enter in: buildForLocale
Look just above the buildForLocale function, notice the Annotation @WithSpan? @WithSpan is an OpenTelemetry Annotation for Java that automatically generates a span around the function that follows.
...
if (!isEnglish(title)) { throw new InvalidLocaleException("Non English Characters found in Instrument Data"); } else { System.out.println("Characters OK "); } To this:
//if (!isEnglish(title)) { // throw new InvalidLocaleException("Non English Characters found in Instrument Data"); // } else { // System.out.println("Characters OK "); //} Save Changes in nano
CTRL-O
Exit nano
CTRL-X
Make sure you saved your changes to shop/src/main/java/com/shabushabu/javashop/shop/model/Instrument.java
Rebuild and Deploy Application ./BuildAndDeploy.sh We are waiting a few minutes…
Return to the service map
...
Click on shop service Click Traces ( right side ) Select “Errors Only” If you do not have red in your service map and you do not see Errors in traces, you have successfully completed our Inventory application review for Sri Lanka and Colorado locations!!
We are nearly done, one more location to go… Chicago Last but not least, let’s ensure Chicago was on-boarded correctly. However, since we have been having so many issues related to “location” and we have added that custom attribute via Opentelemetry Manual Instrumentation, let’s go to the Splunk Observability UI and look at an APM metric set around that tag that I created for us.
...
Click on Traces on the right Select “Errors Only” We can see the exception was thrown by Hibernate, however it was thrown in our method “instruments: InstrumentRepository.findInstruments”
Let’s play developer again Edit the file “instruments: InstrumentRepository.findInstruments”
Using nano
nano instruments/src/main/java/com/shabushabu/javashop/instruments/repositories/FindInstrumentRepositoryImpl.java We can see the developer accidently added the Instruments database with the Chicago Instruments database !
Let’s change the query and fix this, remove “instruments_for_sale” from our Query
Change this:
public Object findInstruments() { LOGGER.info("findInstruments Called (All)"); Object obj = entityManager.createNativeQuery( "SELECT * FROM instruments_for_sale, instruments_for_sale_chicago").getResultList(); return obj; } To this:
// public Object findInstruments() { // LOGGER.info("findInstruments Called (All)"); // Object obj = entityManager.createNativeQuery( "SELECT * FROM instruments_for_sale_chicago").getResultList(); // return obj; //} Save Changes in nano
CTRL-O
Exit nano
CTRL-X
Build and Deploy Application cd javashop-otel directory ./BuildAndDeploy.sh Now let’s test the Chicago location once again
Open a browser and navigate to http://localhost:8010 Select the Chicago Location and Login We now see the 500 error is gone! Let’s confirm a clean Service Map
...
Have a lovely day.
`,description:"",tags:null,title:"6. O11y - Distributed Tracing for modern applications",uri:"/tko/session-6/lab1/index.html"},{content:` 45 minutes During this technical workshop you will learn how to:
...
`,description:"",tags:null,title:"Getting Data In (GDI) \u0026 OTEL",uri:"/tko/session-2/index.html"},{content:`Splunk APM is a NoSample™ Full-fidelity application performance monitoring and troubleshooting solution for cloud-native, microservices-based applications.
...
`,description:"Splunk APM is a NoSample™ Full-fidelity application performance monitoring and troubleshooting solution for cloud-native, microservices-based applications.",tags:null,title:"Splunk APM",uri:"/apm/index.html"},{content:`Aim Escalation policies determine who is actually on-call for a given team and are the link to utilizing any rotations that have been created.
...
What is Locust? ↩︎
`,description:"",tags:null,title:"Deploying NGINX in K3s",uri:"/imt/gdi/nginx/index.html"},{content:`Tag Spotlight On the right hand side of the screen scroll down on Tag Spotlight, ensure Top Across All Indexed Tags is selected in the dropdown click the full screen button as indicated in the screenshot below.
...
`,description:"",tags:null,title:"2.2 Tag Spotlight",uri:"/apm/using-splunk-apm/tag_spotlight/index.html"},{content:` 20 minutes Introduction to the Dashboards and charts Editing and creating charts Filtering and analytical functions Using formulas Saving charts in a dashboard Introduction to SignalFlow 1. Dashboards Dashboards are groupings of charts and visualizations of metrics. Well-designed dashboards can provide useful and actionable insight into your system at a glance. Dashboards can be complex or contain just a few charts that drill down only into the data you want to see.
...
agent: config: receivers: receiver_creator: receivers: smartagent/apache: rule: type == "port" \u0026\u0026 pod.name matches "apache" \u0026\u0026 port == 80 config: type: collectd/apache url: http://php-apache-svc.apache.svc.cluster.local/server-status?auto 3. Observation Rules in the OpenTelemetry config The above file contains an observation rule for Apache using the OTel receiver_creator. This receiver can instantiate other receivers at runtime based on whether observed endpoints match a configured rule.
...
kubectl get cm splunk-otel-collector-otel-agent -n splunk -o yaml Workshop Question Is the content of otel-apache.yaml saved in the ConfigMap for the collector agent?
...
kubectl apply -f ~/workshop/k3s/php-apache.yaml -n apache Workshop Question What metrics for your Apache instance are being reported in the Apache Navigator?
...
kubectl get statefulset -n apache Workshop Question Using the Observability Kubernetes Navigator, can you find the status of the php-apache Workload?
HINT: Filter by k8s.cluster.name to isolate your instance!
Workshop Question Where else has the issue with php-apache been logged? What is being reported?
...
`,description:"",tags:null,title:"Deploying PHP/Apache",uri:"/tko/session-5/deploy-apache/index.html"},{content:`1. Enable RUM For the Real User Monitoring (RUM) instrumentation, we will add the Open Telemetry Javascript https://github.com/signalfx/splunk-otel-js-web snippet in the pages, we will use the wizard again Data Management → Add Integration → RUM Instrumentation → Browser Instrumentation.
...
In case of a message about a VARIABLE being unset Please undeploy the APM environment by running kubectl delete -f deployment.yaml Before exporting the variable as described in the guide and rerunning the deployment script above.
...
`,description:"",tags:null,title:"3. Example of RUM enablement in your Website",uri:"/rum/3-setup/index.html"},{content:`1 Creating a new chart Let’s now create a new chart and save it in our dashboard!
...
`,description:"",tags:null,title:"3.3 Using Filters \u0026 Formulas",uri:"/imt/dashboards/filtering/index.html"},{content:`Please note to begin the following lab, you must have completed the prework:
...
Verify yelp data files are present ll /var/appdata/yelp* Export the following variables. export ACCESS_TOKEN=\u003cyour-access-token\u003e \\ export REALM=\u003cyour-o11y-cloud-realm\u003e \\ export clusterName=\u003cyour-k8s-cluster\u003e Clone the following repo cd /home/ubuntu \\ git clone https://github.com/leungsteve/realtime_enrichment.git \\ cd realtime_enrichment/workshop \\ python3 -m venv rtapp-workshop \\ source rtapp-workshop/bin/activate `,description:"",tags:null,title:"Getting Started with O11y GDI - Real Time Enrichment Workshop",uri:"/tko/session-2/getting_started/index.html"},{content:`This lab will make a tracing superhero out of you!
...
Command Output cd ~ \u0026\u0026 ls o11y-lambda-lab
Note If you don’t see it, fetch the lab contents by running the following command:
git clone https://github.com/kdroukman/o11y-lambda-lab.git 1. Set Environment Variables In your Splunk Observability Cloud Organisation (Org) obtain your Access Token and Realm Values.
...
Export Environment Variables export ACCESS_TOKEN=CHANGE_ME \\ export REALM=CHANGE_ME \\ export PREFIX=$(hostname) 2. Update Auto-instrumentation serverless template Update your auto-instrumentation Serverless template to include new values from the Enviornment variables.
...
Check file contents Expected Content cat ~/o11y-lambda-lab/auto/serverless.yml # USER SET VALUES ===================== custom: accessToken: \u003cupdated to your Access Token\u003e realm: \u003cupdated to your Realm\u003e prefix: \u003cupdated to your Hostname\u003e #====================================== 3. Update Manual instrumentation template Update your manual instrumentation Serverless template to include new values from the Enviornment variables.
...
Check file contents Expected Content cat ~/o11y-lambda-lab/manual/serverless.yml # USER SET VALUES ===================== custom: accessToken: \u003cupdated to your Access Token\u003e realm: \u003cupdated to your Realm\u003e prefix: \u003cupdated to your Hostname\u003e #====================================== 4. Set your AWS Credentials You will be provided with AWS Access Key ID and AWS Secret Access Key values - substitue these values in place of AWS_ACCESS_KEY_ID and AWS_ACCESS_KEY_SECRET in the bellow command:
Set AWS Credentials sls config credentials --provider aws --key AWS_ACCCESS_KEY_ID --secret AWS_ACCESS_KEY_SECRET This command will create a file ~/.aws/credentials with your AWS Credentails populated.
...
Auto-Instrumentation Navigate to the auto directory that contains auto-instrumentation code.
...
Command cat handler.js Can you identify the code for producer function? Can you identify the code for consumer function? Notice there is no mention of Splunk or OpenTelemetry in the code. We are adding the instrumentation using the Lambda layer and Environment Variables only.
...
Command sls info Take note of your endpoint value:
Send some Traffic Use the curl command to send a payload to your producer function. Note the command option -d is followed by your message payload.
...
Producer function logs Producer Function Logs sls logs -f producer Consumer function logs Consumer Function Logs sls logs -f consumer Examine the logs carefully. Do you see OpenTelemetry being loaded? Look out for lines with splunk-extension-wrapper.
Find your Lambda data in Splunk APM Now it’s time to check how your Lambda traffic has been captured in Splunk APM.
...
You should be able to see the producer-lambda and the call it is making to Kinesis service. What about your consumer-lambda? Click into Traces and examine some traces that container procuder function calls and traces with consumer function calls. We can see the producer-lambda putting a Record on the Kinesis stream. But the action of consumer-function is disconnected!
...
Manual Instrumentation Navigate to the manual directory that contains manually instrumentated code.
...
Command cat serverless.yml Do you see any difference from the same file in your auto directory?
...
Inject Trace Context in Producer Function The bellow code executes the following steps inside the Producer function:
...
Re-deploy your Lambdas While remaining in your manual directory, run the following commandd to re-deploy your Lambda Functions:
...
If you see a success message, generate more load: re-send that messate 5+ times. You should keep seeing a success message after each send.
Check the lambda logs output.
Producer Function Logs sls logs -f producer Consumer Function Logs sls logs -f consumer Examine the logs carefully. Do you notice the difference?
...
Find your updated Lambda data in Splunk APM Navigate back to APM in Splunk Observabilty Cloud
...
W3C: https://www.w3.org/TR/trace-context/#traceparent-header B3: https://github.com/openzipkin/b3-propagation#overall-process Which one are we using? It should be self-explanatory from the Propagator we are creating in the Functions
...
Before you Go Please kindly clean up your lab using the following command:
...
`,description:"",tags:null,title:"Build a Distributed Trace in Lambda",uri:"/tko/session-6/lab2/index.html"},{content:`Splunk RUM is the industry’s only end to end, full fidelity Real User Monitoring solution. It is built to optimize performance and aid in faster troubleshooting, giving you full visibility into end-user experiences.
...
`,description:"",tags:null,title:"2.3 Example trace",uri:"/apm/using-splunk-apm/example_trace/index.html"},{content:`1. Introduction Let’s take a look at SignalFlow - the analytics language of Observability Cloud that can be used to setup monitoring as code.
...
2. Fix PHP/Apache Deployment Workshop Question Before we start, let’s check the current status of the PHP/Apache deployment. Under Alerts \u0026 Detectors which [TKO] detector has fired? Where else can you find this information?
...
Workshop Question Another [TKO] Detector has fired, which one is it this time?
...
kubectl edit statefulset php-apache -n apache resources: limits: cpu: "1" memory: 16Mi requests: cpu: 500m memory: 12Mi Save the changes youhave made. (Hint: Use Esc followed by :wq! to save your changes).
...
kubectl describe statefulset php-apache -n apache `,description:"",tags:null,title:"Fix PHP/Apache Issue",uri:"/tko/session-5/fix-apache/index.html"},{content:`1. Introduction For the Splunk Log Observer component, we will configure the Spring PetClinic application to write logs to a file in the filesystem and configure the Splunk OpenTelemetry Collect to read (tail) that log file and report the information to the Splunk Observability Platform.
...
sudo systemctl restart td-agent 3. Logback Settings The Spring PetClinic application can be configure to use a number of different java logging libraries. In this scenario, we are using logback. Here’s a sample logback configuration file:
...
`,description:"",tags:null,title:"4. Log Observer",uri:"/other/pet-clinic/docs/logobserver/index.html"},{content:` Visit the RUM landing page and and check the overview of the performance of all your RUM enabled applications with the Application Summary Dashboard (Both Mobile and Web based) 1. Visit the RUM Landing Page Login into your Splunk IM/APM/RUM Website. From the left side menu bar select RUM . This will bring you to your the RUM Landing Page.
...
`,description:"",tags:null,title:"4. RUM Landing Page",uri:"/rum/4-rum-landing/index.html"},{content:`Objective: Learn how to efficiently deploy complex infrastructure components such as Kafka and MongoDB to demonstrate metrics collection with Splunk O11y IM integrations
Duration: 15 Minutes
...
Infrastructure Monitoring Landing page: K8 Navigator: MongoDB Dashboard: Kafka Dashboard: `,description:"",tags:null,title:"Deploy complex environments and capture meterics",uri:"/tko/session-2/deploy/index.html"},{content:`Splunk Synthetic Monitoring offers the most comprehensive and in-depth capabilities for uptime and web performance optimization as part of the only complete observability suite, Splunk Observability Cloud.
...
`,description:"",tags:null,title:"5. Check Browser Applications health at a glance",uri:"/rum/5-browser-app-summary/index.html"},{content:`Now let’s see how the autoscaler reacts to increased load. To do this, you’ll start a different Pod to act as a client. The container within the client Pod runs in an infinite loop, sending queries to the php-apache Service.
...
cat ~/workshop/k3s/loadgen.yaml This file contains the configuration for the load generator and will create a new StatefulSet with a single replica of the load generator image.
apiVersion: apps/v1 kind: StatefulSet metadata: name: loadgen labels: app: loadgen spec: serviceName: "loadgen" replicas: 1 selector: matchLabels: app: loadgen template: metadata: name: loadgen labels: app: loadgen spec: containers: - name: infinite-calls image: busybox command: - /bin/sh - -c - "while true; do wget -q -O- http://php-apache-svc.apache.svc.cluster.local; done" 2. Create a new namespace kubectl create namespace loadgen 3. Deploy the loadgen YAML kubectl apply -f ~/workshop/k3s/loadgen.yaml --namespace loadgen Once you have deployed the load generator, you can see the Pod running in the loadgen namespace. Use previous similar commands to check the status of the Pod from the command line.
Workshop Question What metrics in the Apache Dashboard have now significantly increased?
...
kubectl scale statefulset/loadgen --replicas 4 -n loadgen Validate the replicas are running from both the command line and Splunk Observability Cloud:
kubectl get statefulset loadgen -n loadgen Workshop Question What impact did this have? Where in O11y can you see the increased replica number?
...
`,description:"",tags:null,title:"Monitoring as Code",uri:"/imt/monitoring-as-code/index.html"},{content:`Code to Kubernetes - Python Objective: Understand activities to instrument a python application and run it on Kubernetes.
...
/home/ubuntu/realtime_enrichment/flask_apps/review/review.py Inspect review.py (workshop/flask_apps_start/review)
...
Use curl in your terminal Or hit the URL http://{Your_EC2_IP_address}:5000 and http://{Your_EC2_IP_address}:5000/get_review with a browser curl localhost:5000 { "message": "Hello, you want to hit /get_review. We have 100000 reviews!" } curl localhost:5000/get_review {"review_id":"NjbiESXotcEdsyTc4EM3fg","user_id":"PR9LAM19rCM_HQiEm5OP5w","business_id":"UAtX7xmIfdd1W2Pebf6NWg","stars":3.0,"useful":0,"funny":0,"cool":0,"text":"-If you're into cheap beer (pitcher of bud-light for $7) decent wings and a good time, this is the place for you. Its generally very packed after work hours and weekends. Don't expect cocktails. \\n\\n-You run into a lot of sketchy characters here sometimes but for the most part if you're chilling with friends its not that bad. \\n\\n-Friendly bouncer and bartenders.","date":"2016-04-12 20:23:24"} Workshop Question What does this application do? Do you see the yelp dataset being used? Why did the output of pip freeze differ each time you ran it? Which port is the REVIEW app listening on? Can other python apps use this same port? }
2. Create a REVIEW container To create a container image, you need to create a Dockerfile, run docker build to build the image referencing the Docker file and push it up to a remote repository so it can be pulled by other sources.
...
curl -s http://localhost:8000/v2/_catalog {"repositories":["review"]} 3. Run REVIEW in Kubernetes Create K8 deployment yaml file for the REVIEW app
...
Access to a Splunk Observability Cloud account in US Splunk Show Your laptop Hopefully a stable wi-fi connection :) In preparation for the workshop, Splunk has prepared an Ubuntu Linux instance in AWS/EC2.
...
`,description:"Make expensive service outages a thing of the past. Remediate issues faster, reduce on-call burnout and keep your services up and running.",tags:null,title:"Splunk OnCall",uri:"/oncall/index.html"},{content:`1. Adding Notes Often on dashboards it makes sense to place a short “instruction” pane that helps users of a dashboard.
...
`,description:"",tags:null,title:"Service Bureau",uri:"/imt/servicebureau/index.html"},{content:`In Kubernetes, a HorizontalPodAutoscaler automatically updates a workload resource (such as a Deployment or StatefulSet), with the aim of automatically scaling the workload to match demand.
...
Workshop Question Which metrics in the Apache Dashboards have significantly increased again?
3. Increase the HPA replica count Increase the maxReplicas to 8
kubectl edit hpa php-apache -n apache Save the changes youhave made. (Hint: Use Esc followed by :wq! to save your changes).
Workshop Question How many pods are now in a running state? How many are pending? Why are they pending?
`,description:"",tags:null,title:"Setup Horizontal Pod Autoscaling (HPA)",uri:"/tko/session-5/setup-hpa/index.html"},{content:`1. Use Data Setup to instrument a Python application Within the O11y Cloud UI:
...
pip install splunk-opentelemetry[all] splunk-py-trace-bootstrap Configure the Downward API to expose environment variables to Kubernetes resources.
...
Update review.deployment.yaml (updates highlighted in bold) apiVersion: apps/v1 kind: Deployment metadata: name: review labels: app: review spec: replicas: 1 selector: matchLabels: app: review template: metadata: labels: app: review spec: imagePullSecrets: - name: regcred containers: - image: localhost:8000/review-splkotel:0.01 name: review volumeMounts: - mountPath: /var/appdata name: appdata env: - name: SPLUNK_OTEL_AGENT valueFrom: fieldRef: fieldPath: status.hostIP - name: OTEL_SERVICE_NAME value: 'review' - name: SPLUNK_METRICS_ENDPOINT value: "http://$(SPLUNK_OTEL_AGENT):9943" - name: OTEL_EXPORTER_OTLP_ENDPOINT value: "http://$(SPLUNK_OTEL_AGENT):4317" - name: OTEL_RESOURCE_ATTRIBUTES value: 'deployment.environment=rtapp-workshop-stevel' volumes: - name: appdata hostPath: path: /var/appdata Apply review.deployment.yaml. Kubernetes will automatically pick up the changes to the deployment and redeploy new pods with these updates
Notice that the review-* pod has been restarted kubectl apply -f review.deployment.yaml deployment.apps/review configured kubectl get pods NAME READY STATUS RESTARTS AGE kafka-client 0/1 Unknown 0 155d curl 0/1 Unknown 0 155d kafka-zookeeper-0 1/1 Running 0 26h kafka-2 2/2 Running 0 26h kafka-exporter-647bddcbfc-h9gp5 1/1 Running 2 26h mongodb-6f6c78c76-kl4vv 2/2 Running 0 26h kafka-1 2/2 Running 1 26h kafka-0 2/2 Running 1 26h splunk-otel-collector-1653114277-agent-n4dfn 2/2 Running 0 26h splunk-otel-collector-1653114277-k8s-cluster-receiver-5f48v296j 1/1 Running 0 26h splunk-otel-collector-1653114277-agent-jqxhh 2/2 Running 0 26h review-6686859bd7-4pf5d 1/1 Running 0 11s review-5dd8cfd77b-52jbd 0/1 Terminating 0 2d10h kubectl get pods NAME READY STATUS RESTARTS AGE kafka-client 0/1 Unknown 0 155d curl 0/1 Unknown 0 155d kafka-zookeeper-0 1/1 Running 0 26h kafka-2 2/2 Running 0 26h kafka-exporter-647bddcbfc-h9gp5 1/1 Running 2 26h mongodb-6f6c78c76-kl4vv 2/2 Running 0 26h kafka-1 2/2 Running 1 26h kafka-0 2/2 Running 1 26h splunk-otel-collector-1653114277-agent-n4dfn 2/2 Running 0 26h splunk-otel-collector-1653114277-k8s-cluster-receiver-5f48v296j 1/1 Running 0 26h splunk-otel-collector-1653114277-agent-jqxhh 2/2 Running 0 26h review-6686859bd7-4pf5d 1/1 Running 0 15s `,description:"",tags:null,title:"Instrument REVIEWS for Tracing",uri:"/tko/session-2/instrument/index.html"},{content:` 45 minutes Here at Buttercup Instruments, our business is expanding ! We have recently added 3 new locations, two locations in the USA, Colorado and Chicago and one international location in SRI Lanka !
...
Now we can see the long latency occurred in the products service and if we click on products: /products we can see the offending method was products:ProductResource.getAllProducts ÷≥« Our next step here would be to send that trace to a developer by clicking download trace and they will have to debug the function. Since we will be the developer there is no need to download the trace. Just remember that is normal workflow for alert responders to route an issue to the “Repairers” while providing trace data.
Before we do that please take note of the Tags available for the developer to leverage to find the root cause. We see standard out of the box Otel tags on the span, environmental information, but no indications of data specific to something inside custom code ( which is where the problem always is. )
Now let’s play the role of the developer As a developer we must debug the function products:ProductResource.getAllProducts to find the problem.
Debugging 101, the Line by Line method Without anything to go on other than “BAD FUNCTION”, a Developer must then look at code visually line by line to find and fix the problem. To make this worse, Functions, call Functions and it can get very messy in bad code scenarios.
We will do the visual inspection method next.
...
Sort by Duration
Select the longest duration trace ( or one of the obvious much longer ones )
NEED SCREEN OF LOCATION TAG “ProductResource.myCoolFunction234234234” was “myInt” with a value of 999 We can see that the actual function call that has the latency was not ProductResource.getAllProducts but the function call “ProductResource.myCoolFunction234234234” !
Since we now have the parameter tagged as part of our span metadata the actual root cause is seemingly related to the “location” Colorado ! And it appears the one custom attribute that was tagged for the function “ProductResource.myCoolFunction234234234” was “myInt” with a value of 999. With this information a Developer can debug very quickly.
Consider the case of debugging code which each of you have just experienced. Imagine that you may have not written the code yourself and the code is a mess, which I can tell you from a former developer, everyone else’s code sucks…. So without Parameter Values at the time of the issue .. You would have no choice but to go line …. by line ….. by line….. which I am sorry I forced you to do earlier… this can take a very long time.
NOTE: We added additional span information, which we call “Custom Attributes” here at Splunk, in this case our “Custom Attributes” are “Parameter Values at Time of Latency”.
In our example the “Location” tag was created due our handy Annotator, that did the OpenTelemetry Annotations for us.
...
If you see a clean service map, free of errors and Latency you have successfully completed the Java Instrumentation Workshop!
Have a lovely day.
`,description:"",tags:null,title:"6. O11y - Distributed Tracing for modern applications",uri:"/tko/session-6/index.html"},{content:` Look into the Metrics views for the various endpoints and use the Tags sent via the Tag spotlight for deeper analysis 1. Find an url for the Cart endpoint From the RUM Overview page, please select the url for the Cart endpoint to dive deeper into the information available for this endpoint.
...
`,description:"",tags:null,title:"8. Analyzing RUM Sessions",uri:"/rum/8-rum-sessions/index.html"},{content:` Continue with the RUM Session information in the RUM UI See correlated APM traces and logs in the APM \u0026 Log Observer UI 1. Finding backend service issues Click on the to close the Span view. Now continue to scroll down and find the POST /cart/checkout line.
...
app: "[nodename]-rum-app", environment: "[nodename]-rum-env" `,description:"",tags:null,title:"10. Custom alerts and charts based on RUM Metrics",uri:"/rum/10-alerting/index.html"},{content:` Getting Data In (GDI) \u0026 OTEL45 minutes During this technical workshop you will learn how to: efficiently deploy complex environments capture metrics from these environments to Splunk Observability Cloud auto-instrument a python application enabling OS logging to Splunk Enterprise via Universal Forwarder In order to simplify the workshop modules, a pre-configured AWS EC2 instance is provided. By the end of this technical workshop you will have an approach to demonstrating metrics collection for complex environments and services.
5. How Platform Engineers Observe Kubernetes45 minutes This workshop will equip you with the basic understanding of monitoring Kubernetes using the Splunk OpenTelemetry Collector. During the workshop you will deploy PHP/Apache and a load generator. You will learn about OpenTelemetry Receivers, Kubernetes Namespaces, ReplicaSets, Kubernetes Horizontal Pod AutoScaling and how to monitor all this using the Splunk Observability Cloud. The main learnings from the workshop will be a better understanding of the Kubernetes Navigator (and Dashboards) in Splunk Observability Cloud as well as seeing Kubernetes metrics, events and Auto-Detectors.
6. O11y - Distributed Tracing for modern applications45 minutes Here at Buttercup Instruments, our business is expanding ! We have recently added 3 new locations, two locations in the USA, Colorado and Chicago and one international location in SRI Lanka ! Our technical staff has already on-boarded the data from these new locations and incorporated them into our inventory application and it is our job to review these improvements and send any issues back to our developers for repairs.
`,description:"TKO 2023 - Observability Workshops",tags:null,title:"TKO Workshops",uri:"/tko/index.html"},{content:` Short introduction to Mobile RUM See an overview of the performance of your Mobile Application(s) in the Application Summary Dashboard 1. Visit your RUM Application Summary Dashboard Visit and login into your Splunk IM/APM/RUM Website. From the left side menu bar select RUM . This will bring you to your RUM Application Summary Page.
...
`,description:"",tags:null,title:"11. Introdcution to RUM for Mobile Applications",uri:"/rum/11-mobile-app-summary/index.html"},{content:` PetClinic Java WorkshopA workshop using Zero Configuration Auto-Instrumentation for Java
`,description:"",tags:null,title:"Other Workshops",uri:"/other/index.html"},{content:"",description:"",tags:null,title:"Categories",uri:"/categories/index.html"},{content:"",description:"",tags:null,title:"Tags",uri:"/tags/index.html"}]
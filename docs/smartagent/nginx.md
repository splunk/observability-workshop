...
Validate you are seeing metrics in the UI by going to **Dashboards → NGINX → NGINX Servers**. Using the **Overrides** filter on `kubernetes_cluster:` and find the name of your cluster as returned by `echo $(hostname)-k3s-cluster` in the terminal.

...
Once you have finished with this workshop `exit` from the Multipass instance(s) you are in and get back to your system command prompt and enter the following to delete the Multipass instance(s), replace `[YOUR_INITIALS]` with the ones you used in [Integrations - Step #2.3](../../victorops/vo_integrations/#23-local-vms-using-multipass):

...
runcmd:
  ...
  - k3d cluster create ${HOSTNAME}-cluster --agents 2 --agents-memory 6G --k3s-arg "--kubelet-arg=eviction-hard=memory.available<1Gi@server:0" --k3s-arg "--kubelet-arg=eviction-hard=memory.available<1Gi@agent:*" --image rancher/k3s:v1.33.4-k3s1 --port "80:80@loadbalancer" --port "81:80@loadbalancer" --port "82:82@loadbalancer" -v /home/splunk:/home/splunk -v /var/log/syslog:/var/log/syslog -v /var/log/auth.log:/var/log/auth.log

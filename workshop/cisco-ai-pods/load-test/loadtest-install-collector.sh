...
usage() {
  cat <<'EOF'
Usage:
  ./loadtest-install-collector.sh --access-token TOKEN --hec-url URL --hec-token TOKEN --cluster-api URL --password PASS [options]
...
install_for_user () {
  ...
  echo "[$user] helm install..."
  KUBECONFIG="$kubeconfig" helm upgrade --install "$release" \
    --namespace "$namespace" \
  ...
  echo "[$user] waiting for DaemonSet rollout + pod readiness..."
  KUBECONFIG="$kubeconfig" kubectl -n "$namespace" rollout status ds/splunk-otel-collector-agent --timeout=10m
  KUBECONFIG="$kubeconfig" kubectl -n "$namespace" wait --for=condition=Ready pod --all --timeout=10m
  echo "[$user] done"
}

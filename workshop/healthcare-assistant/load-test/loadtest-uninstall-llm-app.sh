...
usage() {
  cat <<'EOF'
Usage:
  ./loadtest-uninstall-llm-app.sh --csv FILE [options]
...
cd ~/workshop/healthcare-assistant/1-base-app
kubectl delete -f setup-job.yaml || true

kubectl delete secret galileo-secret || true
kubectl delete secret openai-api || true
kubectl delete cm galileo-agent-control-config || true
kubectl delete cm galileo-config || true
kubectl delete cm healthcare-assistant-config || true
...
if (( fail )); then
  echo "One or more remote uninstalls FAILED."
  exit 1
fi

echo "All remote uninstalls completed successfully."
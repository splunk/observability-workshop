...
`,description:"Online BoutiqueアプリケーションをKubernetes（K3s）にデプロイし、Locustを使って人工的なトラフィックを発生させます。",tags:null,title:"1. Online Boutiqueのデプロイ",uri:"/ja/apm/online-boutique/index.html"},{content:`RUM ワークショップの概要 このSplunk Real User Monitoring (RUM) ワークショップの目的は以下の通りです。
...
`,description:"",tags:null,title:"チーム",uri:"/ja/imt/servicebureau/teams/index.html"},{content:`この テクニカル Splunk Observability Cloud ワークショップでは、 lightweight Kubernetes1 クラスタをベースにした環境を構築します。
...
`,description:"",tags:null,title:"API Checks",uri:"/ja/synthetics/api-checks/index.html"},{content:` 15分 Splunk Helm chartを使用して、K3s に OpenTelemetry Collector をインストールします Kubernetes Navigatorでクラスタを探索します 1. Access Tokenの取得 Kubernetes が起動したら、Splunk の UI から Access Token1 を取得する必要があります。Access Token は、左下にある » を開き、 Settings → Access Tokens を選択すると表示されます。
...
`,description:"",tags:null,title:"ダッシュボードを利用する",uri:"/ja/imt/dashboards/index.html"},{content:` ブラウザでOnline BoutiqueのウェブページのオリジナルのHEADセクション（またはここにある例を使用）をチェックします ワークショップ Online Boutique の Webアドレスを検索します Online Boutiqueに加えられた変更を確認します 1. RUMなしのOnline Boutiqueのオリジナルコードを確認する APMセッションの一部でEC2インスタンスにOnline Boutiqueをインストールしていれば、ポート番号81でサイトにアクセスできます。
...
`,description:"",tags:null,title:"ディテクターを利用する",uri:"/ja/imt/detectors/index.html"},{content:` RUMランディングページにアクセスし、アプリケーションサマリーダッシュボード（モバイルおよびウェブベース）でRUM対応アプリケーションすべてのパフォーマンスの概要を確認します。 1. RUMランディングページにアクセス Splunk IM/APM/RUM ウェブサイトにログインします。左側のメニューバーから RUM を選択します。 RUMのランディングページが表示されます。
...
`,description:"",tags:null,title:"11. モバイルアプリケーション (紹介)",uri:"/ja/rum/11-mobile-app-summary/index.html"},{content:"",description:"",tags:null,title:"Categories",uri:"/ja/categories/index.html"},{content:"",description:"",tags:null,title:"Tags",uri:"/ja/tags/index.html"}]
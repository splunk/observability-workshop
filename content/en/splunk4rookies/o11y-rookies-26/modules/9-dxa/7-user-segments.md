...
**User segments** group users by attributes (device type, location), session properties, or behaviors. Segments let us run targeted analyses — for example, comparing checkout conversion on mobile versus desktop — without rebuilding filters each time.
...
We have now explored the core DXA workflow. Let's connect what we learned to business outcomes.

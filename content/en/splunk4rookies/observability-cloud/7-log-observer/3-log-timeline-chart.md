...
* Click on the Configure Table {{< icon "settings" >}} icon above the **Logs table** to open the **Table Settings**, untick `_raw` and ensure the following fields are selected `k8s.pod.name`, `message` and `version`.
...
* To make this work for all traces, remove the `trace_id` from the filter and add the fields `service.name=paymentservice` and `sf_environment=[WORKSHOPNAME]`.
...
* In the **New dashboard** dialog box, enter a name for the new dashboard (no need to enter a description). Use the following format: `Initials - Service Health Dashboard` and click {{% button style="blue" %}}Save{{% /button %}}. Make a note of your dashboard name, we will need it in a short while. 

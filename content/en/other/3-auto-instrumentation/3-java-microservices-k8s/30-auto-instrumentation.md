...
Due to the time it takes for the PetClinic microservices application to sync and get into a working order, we are going to trigger auto instrumentation for the three management microservices of the Petclinic Java Application now as these services need to be up and running before the actual application can run. Let's run a script that will patch these three services and annotate them so the OpenTelemetry operator will initialize the zero-config instrumentation and start instrumenting them.
...
If you want your pods to send traces automatically, you can add the annotation to the `deployment.yaml` as shown below. This will add the instrumentation library during the initial deployment.
...
However, the more common scenario is that you need to see the full interaction between all services. So let's patch all the deployments (labeled with `app.kubernetes.io/part-of=spring-petclinic`) to add the inject annotation.
...
=======
In the Dependency map, make sure you have the `api-gateway` service selected as shown above, then click on the ***View Dashboard** Link **(1)**  at the top of the right-hand pane.
>>>>>>> 88182dfee (Grammarly fixes)
...
=======
This dashboard, which is available for each of your instrumented services, offers an overview of the key `RED` performance Metrics.
As the dashboards allow you to go back in time with the *Time picker* window **(1)**, it's the perfect spot to identify the behavior you wish to be alerted of, and with a click on one of the bell icons **(2)** in each chart you can set up an alert to do just that.
>>>>>>> 88182dfee (Grammarly fixes)

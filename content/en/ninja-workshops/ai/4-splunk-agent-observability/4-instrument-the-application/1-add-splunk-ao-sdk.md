...
Run the following command to create a Kubernetes secret, which stores the Splunk Agent Observability credentials: 

```bash
kubectl create secret generic splunk-ao-secret \
  --from-literal=realm="$SPLUNK_AO_REALM" \
  --from-literal=o11y-token="$SPLUNK_AO_O11Y_TOKEN" \
  --from-literal=o11y-api-token="$SPLUNK_AO_O11Y_API_TOKEN"
```
...
`setup_env.py` exports empty `SPLUNK_AO_PROJECT` / `SPLUNK_AO_AGENT_STREAM` values, and the

...
The log is always available in the console and `agent-logs.out`. The default
log pipeline also contains Splunk's HEC exporters. If HEC was skipped during
setup, connection errors for the local placeholder endpoint can be ignored.
Sending the workshop logs to your own non-production Splunk Platform instance
is a take-home exercise.

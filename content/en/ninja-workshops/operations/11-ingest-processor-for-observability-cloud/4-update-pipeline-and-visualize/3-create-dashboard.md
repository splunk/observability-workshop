...
linkTitle: 4.3 Visualize Kubernetes Audit Event Metrics
weight: 4
---

Now that you have added dimensions, aggregated your metric, and dropped the noisy `auditID` dimension, you will create a chart showing the health of different Kubernetes actions using the aggregated metric.

{{% notice title="Exercise: Visualize Kubernetes Audit Event Metrics" style="green" icon="running" %}}

**1.** In the upper-right corner, click the **+** Icon → **Chart** to create a new chart. In the **Plot Editor** enter `k8s_audit_UNIQUE_ID.without.auditID.agg` in the metric name field. Notice that you're using the newly created aggregated metric for this chart.

![Create New Chart](../../images/create_new_chart.png?width=40vw)

Now that you have a clean, lower-cardinality metric available, let's adjust the chart to show us if any of our actions have errors associated with them.
...
**2.** In the plot editor of your chart click the **Add filter**, use **response_status** for the field and select **409.0** and **503.0** for the values.
...
**3.** Click **Add analytics** → **Sum** → **Sum:Aggregation** and add **resource**, **action**, and **response_status** in the **Group by** field.

![Add Metric Filters](../../images/add_metric_filters.png?width=40vw)

**4.** Using the chart type along the top buttons, change the chart to a **heatmap**. Next to the **Plot editor**, click **Chart options**. In the **Group by** section select **response_status** then **action**. Change the **Color threshold** from **Auto** to **Fixed**. Click the blue **+ button** to add another threshold. Change the **Down arrow to Yellow**, the **Middle to orange**. Leave the **Up arrow as red**. Enter **1 for the middle threshold** and **5 cpm for the upper threshold**.

![Configure Thresholds](../../images/configure_thresholds.png?width=40vw)

**5.** In the upper right corner of the chart click the blue **Save as...** ![Preview Button](../../images/save_as_btn.png?height=20px&classes=inline) button. Enter a name for your chart (For Example: Kubernetes Audit Logs - Conflicts and Failures).

![Chart Name](../../images/chart_name.png)

**6.** On the **Choose a dashboard** select **New dashboard**.

![New Dashboard](../../images/new_dashboard.png)

**7.** Enter a name for your dashboard that includes your initials, so you can easily find it later. Click **Save**.

![New Dashboard Name](../../images/dashboard_name.png)

**8.** Make sure the new dashboard you just created is selected and click **Ok**.
...
{{% notice title="Optional Exercise: Create a detector based on Kubernetes Audit Logs" style="green" icon="running" %}}
...
{{% notice title="Optional Exercise: Visualize your time series data in Splunk Cloud - Dashboard Studio" style="green" icon="running" %}}

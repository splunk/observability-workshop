---
title: Understanding Critical User Journeys
description: Optimize and troubleshoot end user experience with Splunk Observability Cloud.
weight: 2
archetype: chapter
authors: ["Sarah Ware"]
time: 60 minutes
hidden: false
---

Explore how to get insight to end user experience with Splunk Observability Cloud, and how it connects the front end to underlying application issues for better context and faster MTTx. 

## Workshop Overview

During this workshop, we will cover the following topics:

- Understand end user behavior with Digital Experience Analytics (DXA)
- See what app users see with Session Replay
- Monitor performance for real users with Real User Monitoring (RUM)
- Proactively test frontend performance with Synthetics

By the end of this workshop, you'll be familiar with understanding application health and success through the lens of end user experience.

{{< tabs >}}
{{% tab title="Question" %}}
Why is it important to understand end user experience?
{{% /tab %}}
{{% tab title="Answer" %}}
We measure so that we can improve, and we improve in order to better serve our end users - and ultimately to better achieve our business goals. Happy app users are good for business! 
{{% /tab %}}
{{< /tabs >}}

Before we dive into Observability, let's step into the shoes of our end users.
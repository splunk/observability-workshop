...
このワークショップでは、`The Door Game` というJavaベースのアプリケーションを使用します。このアプリケーションはKubernetesでホストされます。

## 前提条件

EC2インスタンスから開始し、以下の状態にするためにいくつかの[初期ステップ](#initial-steps)を実行します

* **Splunk distribution of the OpenTelemetry Collector** をデプロイする
* MySQLデータベースコンテナをデプロイしてデータを投入する
* `doorgame` アプリケーションコンテナをビルドしてデプロイする

## 初期ステップ

初期セットアップは、EC2インスタンスのコマンドラインで以下のステップを実行することで完了できます。
...
アプリケーションがデプロイされたので、実際にプレイしてオブザーバビリティデータを生成しましょう。

ブラウザでEC2インスタンスのIPアドレスのポート81にアクセスすると、The Door Game アプリケーションにアクセスできます。例
...
`Let's Play` をクリックしてから実際にゲームを開始できるまで、長い時間がかかったことに気づきましたか？

**Splunk Observability Cloud** を使用して、アプリケーションの起動がなぜ遅いのかを調べましょう。

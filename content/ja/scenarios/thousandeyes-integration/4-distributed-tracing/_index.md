...
以下の例では、このワークショップに含まれる Spring PetClinic アプリケーションを使用します。ThousandEyes のトレース相関に必要な Kubernetes アノテーション、`Instrumentation` リソース、および Pod レベルの設定を示します。

### 1. デプロイメントのアノテーション

このガイドでは、PetClinic の Java デプロイメントが `default/splunk-otel-collector` Instrumentation リソースを指します。
...
これは PetClinic の `Instrumentation` オブジェクトで、ThousandEyes に関係するフィールドのみに絞り込んだものです。
...
実行中の PetClinic `api-gateway` Pod で、Operator が期待される OpenTelemetry 設定を注入していることを確認します。
...
{{% notice title="PetClinic からの教訓" style="info" %}}
PetClinic の `Instrumentation` リソースは、`b3` と `tracecontext` を明示的に組み合わせているため、ThousandEyes で使用するパターンになります。このシナリオではこの設定を使用します。
{{% /notice %}}
...
PetClinic の API gateway だけをパッチする必要がある場合は、PetClinic の namespace 修飾された Instrumentation リソースを指定します。

```bash
kubectl patch deployment api-gateway -n default \
  -p '{"spec":{"template":{"metadata":{"annotations":{"instrumentation.opentelemetry.io/inject-java":"default/splunk-otel-collector"}}}}}'
```
...
2. PetClinic API gateway に対して、いくつかのリクエストを生成します。

   ```text
   http://api-gateway.default.svc.cluster.local:82/api/customer/owners
   ```

   このリクエストは PetClinic API gateway から入り、`customers-service` にルーティングされ、PetClinic データベースを照会します。シンプルなヘルスチェックよりも有用なトレースを生成します。

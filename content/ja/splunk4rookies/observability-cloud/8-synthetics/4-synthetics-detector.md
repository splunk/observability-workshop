...
- 残りはデフォルトのままにします。
- スパイクの下に図形の列が表示されるようになったことに注意してください（**3**）。塗りつぶされた図形は、テストが指定されたしきい値を超えたことをDetectorが検出したことを知らせ、輪郭のみの図形は結果がしきい値を下回ったことを示します。各図形がアラートをトリガーします。
...
- 新しく作成したDetectorを見るには、**More actions** をクリックし、次に **Edit test** をクリックします。

...
RUMウォーターフォールでAPMリンクをクリックすると、その **Business Transaction**（`frontend:/cart`）に関連するサービスを表示するために、サービスマップビューに自動的にフィルターが追加されました。

ワークフローに関連するサービスは **Service Map** で確認できます。サイドペインの **Business Transaction** の下には、選択したワークフローのチャートが表示されています。**Service Map** と **Business Transaction** チャートは同期しています。**Service Map** でサービスを選択すると、**Business Transaction** ペインのチャートが更新され、選択したサービスのメトリクスが表示されます。

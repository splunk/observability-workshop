...
ここまでで、Splunk Distribution of the OpenTelemetry Collector をデフォルト設定で K8s クラスターにデプロイしてきました。このセクションでは、Collector 設定をカスタマイズする方法をいくつかの例を通じて説明します。

## Collector 設定の取得

Collector 設定をカスタマイズする前に、現在の設定がどのようになっているかを確認するにはどうすればよいでしょうか。

Kubernetes 環境では、Collector の設定は Config Map を使用して保存されます。

クラスター内に存在する config map は、次のコマンドで確認できます。
...
Linux インスタンスで Collector を実行していた以前の例では、Collector の設定は `/etc/otel/collector/agent_config.yaml` ファイルにありました。その場合に Collector 設定を変更する必要があれば、単にこのファイルを編集して変更を保存し、Collector を再起動するだけです。

K8s では、少し動作が異なります。`agent_config.yaml` を直接変更するのではなく、helm chart のデプロイに使用する `values.yaml` ファイルに変更を加えることで Collector 設定をカスタマイズします。
...
最初の例として、K8s クラスターに対して infrastructure events monitoring を有効化してみましょう。

> これにより、Kubernetes イベントをチャートの Events Feed セクションの一部として確認できるようになります。
> cluster receiver は、kubernetes-events モニターを使用する Smart Agent receiver で構成され、カスタムイベントを送信します。詳細は [Collect Kubernetes events](https://docs.splunk.com/observability/en/gdi/opentelemetry/collector-kubernetes/kubernetes-config-logs.html#collect-kubernetes-events) を参照してください。

これは、`values.yaml` ファイルに以下の行を追加することで実行されます。

> ヒント: vi で開いて保存する手順は、以前の手順にあります。
...
その後、config map を確認して変更が適用されたことを確認できます。
...
`smartagent/kubernetes-events` がエージェント設定に含まれていることを確認します。
...
> これらの特定の変更が適用される場所であるため、cluster receiver の config map を指定したことに注意してください。

## Debug Exporter の追加

Collector に送信されるトレースとログを確認したい場合を考えてみましょう。Splunk に送信する前にそれらを検査できるようにするためです。この目的には debug exporter を使用できます。これは OpenTelemetry 関連の問題のトラブルシューティングに役立ちます。

values.yaml ファイルの末尾に、以下のように debug exporter を追加してみましょう。
...
エージェント Collector のログには、次のようなトレースが書き込まれているのが確認できるはずです。
...
ただし、Splunk Observability Cloud に戻ると、アプリケーションからトレースとログが送信されなくなっていることに気付くでしょう。

なぜそうなったのでしょうか? 次のセクションで詳しく見ていきます。

...
EC2インスタンスには、Splunk Distribution of the OpenTelemetry Collectorの古いバージョンがすでにインストールされている可能性があります。先に進む前に、以下のコマンドでアンインストールしましょう。
...
最新バージョンのSplunk Distribution of the OpenTelemetry CollectorをLinuxのEC2インスタンスへデプロイしましょう。

`curl`を使ってcollectorのバイナリをダウンロードし、データを送信するrealm、使用するアクセストークン、送信先のデプロイ環境を指定する引数を付けて実行します。

> Splunk Observability Cloudのデプロイ環境とは、システムやアプリケーションの個別のデプロイを指します。同じアプリケーションの他のデプロイの設定と重複しない構成を設定できます。
...
collectorのインストール方法に関する詳細は、[Install the Collector for Linux with the installer script](https://docs.splunk.com/observability/en/gdi/opentelemetry/collector-linux/install-linux.html#otel-install-linux)を参照してください。

## Collectorが起動していることの確認

collectorがインスタンス上で正常に起動していることを確認しましょう。

> Ctrl + Cキーを押すとstatusコマンドを終了できます。
...
`journalctl`を使ってcollectorのログを確認できます。

> Ctrl + Cキーを押すとログのtail表示を終了できます。
...
このcollectorが使用する構成はどこにあるのでしょうか？

`/etc/otel/collector`ディレクトリにあります。collectorを`agent`モードでインストールしたので、collectorの構成は`agent_config.yaml`ファイルにあります。

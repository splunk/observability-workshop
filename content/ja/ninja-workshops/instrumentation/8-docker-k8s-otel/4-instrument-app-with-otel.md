...
このワークショップでは、NuGet パッケージを使用せずに、Splunk Distribution of OpenTelemetry を手動でインストールします。
...
デプロイメント環境を設定して、データが Splunk Observability Cloud 内の専用環境に送信されるようにしましょう:
...
1. `dotnet run` コマンドの先頭に `OTEL_TRACES_EXPORTER=otlp,console` を追加することで、トレースが OTLP 経由でコレクターとコンソールの両方に書き込まれるようになります。

``` bash
OTEL_TRACES_EXPORTER=otlp,console dotnet run 
```

2. または、コレクター設定に debug エクスポーターを追加し、トレースパイプラインに追加することで、トレースがコレクターのログに書き込まれるようになります。
...
トレースログを有効にしている場合、以下のようなトレースがコンソールまたはコレクターのログに書き込まれるのが確認できるはずです:

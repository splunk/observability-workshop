...
このワークショップではシンプルな「Hello World」.NETアプリケーションを使用します。メインのロジックはHelloWorldController.csファイルにあります。
...
アプリケーションが起動したら、2つ目のSSHターミナルをUbuntuインスタンスに開き、curlを使用してアプリケーションにアクセスします。
...
OpenTelemetryでアプリケーションを計装するために使用できる3つの方法は何でしょうか？

![Traces](../images/NetInstrumentation.png)

オプションの詳細については [Instrument your .NET application for Splunk Observability Cloud](https://docs.splunk.com/observability/en/gdi/get-data-in/application/otel-dotnet/instrumentation/instrument-dotnet-application.html) を参照してください。

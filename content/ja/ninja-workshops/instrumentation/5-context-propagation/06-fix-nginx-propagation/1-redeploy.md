...
ファイルを保存しただけでは、実行中のgatewayは更新 **されません** 。ConfigMapを適用し、デプロイメントを再起動してNGINXに新しい設定を読み込ませる必要があります。
...
AGEが最近であることを確認します（設定変更後にPodが再作成されています）。
...
#### 3. traceparentがorder-apiに到達していることを確認
...
`traceHint.traceId` が `traceparent` ヘッダーで送信したトレースIDと一致していることを確認します。これにより、gatewayが `order-api` にヘッダーを転送したことが確認できます。
...
- `deploy/k8s/gateway-config.yaml` を **保存** し、 **`make fix-nginx`** （または手動の `kubectl apply` + restart コマンド）を実行したことを確認します
- gatewayのPodが再起動したことを確認します: `kubectl -n cosmic-shop get pods -l app=gateway`
...
#### 問題2: ConfigMapは更新されたがPodに古い設定が表示される

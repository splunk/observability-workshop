...
---
このステップでは、k3dを使用してローカルコンテナレジストリを備えた軽量なKubernetesクラスターを作成します。
...
すべてのノードのSTATUS列に `Ready` と表示されていることを確認します。
...
ポート30080または5111が使用中の場合、競合するサービスを停止するか、`scripts/setup-k3d.sh` を編集して別のポートを使用します。
...
そのターミナルを開いたまま、<http://localhost:15672> を開きます（`guest` / `guest`）。

#### 問題3: クラスターが既に存在する

`cosmic-shop` という名前のクラスターが既に存在する場合、スクリプトは作成をスキップします。最初からやり直すには:

...
**エージェントを再起動します**: **Agent ターミナル**ウィンドウを見つけて、変更した設定を使用して `agent` を（再）起動します
...
**トレースを送信します**: **Spans ターミナル**ウィンドウから、別のスパンを送信し、以前と同じ出力がコンソールに表示されることを確認します

```bash { title="Start Load Generator" }
../loadgen -count 1
```

これで、`agent.out` ファイルが書き込まれたことを確認するために、**Agent ターミナル**ウィンドウで `Ctrl-C` を使用して `agent` を停止できます。

**`agent.out` ファイルが書き込まれたことを確認します**: 現在のディレクトリに `agent.out` という名前のファイルが書き込まれていることを確認します。

```text { title="Updated Directory Structure" }
.
├── agent.out    # File Exporter によって作成された OTLP/Json 出力
└── agent.yaml
```

**スパンのフォーマットを確認します**:

1. File Exporter が `agent.out` にスパンを書き込む際に使用するフォーマットを確認します。
2. 出力は **OTLP/JSON** フォーマットの単一行になります。

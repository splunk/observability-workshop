---
title: 1.3 AgentからGatewayへトレースを送信する
...
{{% exercise title="テストトレースの送信" %}}

**テストトレースの送信**

1. **Agent** と **Gateway** がまだ実行中であることを確認します。
2. **Loadgenターミナル** ウィンドウで、以下のコマンドを実行して5つのSpanを送信し、**Agent** と **Gateway** のデバッグログの出力を確認します。
...
**GatewayがSpanを処理したことを確認する**: Gatewayが受信したSpanを処理すると、トレースデータを `gateway-traces.out` という名前のファイルに書き込みます。Spanが正常に処理されたことを確認するには、このファイルを検査します。

**Testsターミナル** で、`jq` コマンドを使用して各Spanの主要な詳細情報（`spanId` やファイル内の位置など）を抽出して表示します。また、**Hostmetrics Receiver** がSpanに追加した属性も抽出できます。

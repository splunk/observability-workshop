---
title: 1.2 Agent設定のテスト
linkTitle: 1.2 Agent設定のテスト
weight: 2
---

新しく作成した `agent.yaml` を使用してOpenTelemetry Collectorを起動する準備ができました。この演習では、OpenTelemetry Collectorを通じてデータがどのように流れるかを理解するための基礎を築きます。

{{% notice title="Exercise" style="green" icon="running" %}}

**Agentの起動**: **Agent terminal** ウィンドウで以下のコマンドを実行します:

```bash { title="Start Collector" }
../otelcol --config=agent.yaml
```

**デバッグ出力の確認**: すべてが正しく設定されている場合、出力の最初と最後の行は以下のようになります:
...
**テストSpanの送信**: アプリケーションを計装する代わりに、`loadgen` ツールを使用してOpenTelemetry Collectorにトレースデータを送信するシミュレーションを行います。

**Spans terminal** ウィンドウで `1-agent` ディレクトリに移動し、以下のコマンドを実行して単一のSpanを送信します:
...
`{"partialSuccess":{}}`: `partialSuccess` フィールドが空であるため、100%成功したことを示します。部分的な失敗が発生した場合、このフィールドには失敗した部分の詳細が含まれます。
...
**Agent terminal** ウィンドウでCollectorのデバッグ出力を確認します:
...
> [!IMPORTANT]
> **Agent terminal** ウィンドウで `Ctrl-C` を使用して `agent` を停止します。

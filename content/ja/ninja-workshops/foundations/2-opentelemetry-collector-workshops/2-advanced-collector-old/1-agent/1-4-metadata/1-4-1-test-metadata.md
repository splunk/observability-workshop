...
**Agentを再起動する**: **Agentターミナル** ウィンドウで、更新した設定を使用して `agent` を再起動し、変更をテストします:

```bash { title="Start the Agent" }
../otelcol --config=agent.yaml
```

すべてが正しく設定されていれば、出力の最後の行でCollectorが実行中であることが確認できます:

```text
  2025-01-13T12:43:51.747+0100 info service@v0.120.0/service.go:261 Everything is ready. Begin running and processing data.
```

**Traceを送信する**: **Spansターミナル** ウィンドウから（`1-agent` ディレクトリにいることを確認して）、`loadgen` バイナリで再度Spanを送信し、新しい `agent.out` を作成します:

```bash { title="Start Load Generator" }
../loadgen
```

**Agentのデバッグ出力を確認する**: `resource attributes` セクションに3つの新しい行が表示されます（`host.name`、`os.type`、`otelcol.service.mode`）:
...
**メタデータがSpanに追加されていることを確認する**: `Ctrl-C` で `loadgen` を停止します。新しい `agent.out` ファイルで以下を確認します:

1. `resourceSpans` セクションに `otelcol.service.mode` 属性が存在し、値が `agent` であることを確認します。
2. `resourcedetection` の属性（`host.name` と `os.type`）も存在することを確認します。

これらの値は、パイプラインで設定されたProcessorによって、デバイスの情報に基づいて自動的に追加されます。
...
> [!IMPORTANT]
> それぞれのターミナルウィンドウで `Ctrl-C` を使用して、`agent` と `loadgen` のプロセスを停止してください。

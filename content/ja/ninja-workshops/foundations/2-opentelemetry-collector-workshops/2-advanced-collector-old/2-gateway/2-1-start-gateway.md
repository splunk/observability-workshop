---
title: 2.1 Gatewayの起動
linkTitle: 2.1 Gatewayの起動
weight: 1
---

`gateway`の設定は、動作させるために追加の設定変更を必要としません。これは時間を節約し、**Gateway** の核心的なコンセプトに焦点を当てるためです。

**[otelbin.io](https://www.otelbin.io/)** を使用して`gateway`の設定を検証します。参考として、パイプラインの`logs:`セクションは以下のようになります。
...
**Gatewayの起動**: **Gatewayターミナル** ウィンドウで、以下のコマンドを実行して`gateway`を起動します。

```bash {title="Start the Gateway"}
../otelcol --config=gateway.yaml
```

すべてが正しく設定されている場合、出力の最初と最後の行は以下のようになります。

...
Receiverでキャプチャされたメトリクスを定義しエクスポートするために、[**mdatagen**](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/cmd/mdatagen) を使用します。これはCollector向けに開発されたツールで、YAMLで定義されたメトリクスをコードに変換します。
...
次のセクションに進む前に、これらのファイルをプロジェクトフォルダ内に作成します。

## Factoryの構築

Factoryはソフトウェアデザインパターンの一つで、提供された設定に基づいてオブジェクト（この場合は `jenkinscireceiver`）を動的に作成できるようにするものです。より現実的な例で説明すると、携帯電話ショップに行き、希望する仕様に合った電話を注文し、それを受け取るようなものです。

次のコマンド `go generate -x ./...` を実行します。これにより、新しいフォルダ `jenkinscireceiver/internal/metadata` が作成され、定義されたメトリクスをエクスポートするために必要なすべてのコードが含まれます。必要なコードは以下のとおりです。
...
これらのファイルを期待される内容でプロジェクトに書き込んだら、`go mod tidy` を実行します。これにより、すべてのリモート依存関係が取得され、`go.mod` が更新され、`go.sum` ファイルが生成されます。

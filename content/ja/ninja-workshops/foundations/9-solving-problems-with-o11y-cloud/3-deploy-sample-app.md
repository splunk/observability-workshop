...
> 用語に関する注意事項です。このワークショップでは**タグ**について説明しており、これは **Splunk Observability Cloud** で使用される用語ですが、OpenTelemetry では代わりに**属性（attributes）**という用語を使用します。このワークショップでタグという言葉が出てきた場合、属性と同義として扱ってください。
...
トレースには、コードでキャプチャしたタグ（属性）である `credit.score` や `credit.score.category` が含まれていることに注目してください。次のセクションでは、Splunk Observability Cloud でトレースを分析し、パフォーマンス問題の根本原因を特定する際にこれらを使用します。

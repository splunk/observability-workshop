...
このステップでは、Portworx のメトリクスエンドポイントを模倣する Python サービスをデプロイします。
これは、ワークショップで Pure Storage の監視を設定するために使用します。
...
Portworx メトリクスエンドポイントが想定どおりに動作していることを確認しましょう。

curl コマンドを利用できる Pod を起動します。

``` bash
oc run --rm -it -n default curl --image=curlimages/curl:latest -- sh
```

続いて、以下のコマンドを実行してエンドポイントにリクエストを送信します。

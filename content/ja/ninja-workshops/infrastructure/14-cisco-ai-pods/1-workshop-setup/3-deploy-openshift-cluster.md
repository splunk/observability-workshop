...
> [!NOTE]
> EXPORT コマンドを実行する前に、Subnet ID と OIDC ID を必ず入力してください
...
> ここでは `g5.4xlarge` インスタンスタイプを指定していることに注意してください。このインスタンスタイプには、
> ワークショップの後半で使用する NVIDIA GPU が含まれています。このインスタンスタイプは比較的高価で、
> 執筆時点で1時間あたり約 $1.64 かかります。また、2つのレプリカを要求しているため、
> クラスターの稼働時間に注意してください。コストはすぐに蓄積されます。

クラスターが Ready 状態になったかどうかを確認するには、以下を実行します
...
> Note: `rosa describe cluster -c $CLUSTER_NAME` コマンドを実行し、
> 返された API Server URL を以下のコマンドに代入してから実行してください。例えば、
> サーバー名は `https://api.rosa-test.aaa.bb.openshiftapps.com:443` のようになります。

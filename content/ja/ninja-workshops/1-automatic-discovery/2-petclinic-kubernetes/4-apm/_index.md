...
このセクションでは、Kubernetes 上で実行されている Java サービスに対して**自動検出と設定**を有効化します。これにより、OpenTelemetry Collector が Pod アノテーションを検索し、Java アプリケーションに Splunk OpenTelemetry Java エージェントでインストゥルメンテーションを行う必要があることを示します。これにより、クラスター上で実行されている Java サービスからトレース、スパン、およびプロファイリングデータを取得できるようになります。
...
これは APM を始めるための優れた方法ですが、手動インストゥルメンテーション の**代替ではありません**。手動インストゥルメンテーションでは、カスタムスパン、タグ、ログをアプリケーションに追加でき、トレースにより多くのコンテキストと詳細を提供できます。
...
Pod が自動的にトレースを送信するようにしたい場合は、以下に示すように`deployment.yaml`にアノテーションを追加できます。これにより、初期デプロイメント時にインストゥルメンテーションライブラリが追加されます。時間を節約するために、以下の Pod に対してこれを実施済みです：

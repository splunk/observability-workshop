...
このカスタム設定により、エクスポーターが受信したヒストグラムメトリクスは、SignalFx 形式に変換されずに OTLP 形式で Splunk Observability バックエンドに送信されます。この設定は、`gen_ai.evaluation.score` などの AI Agent Monitoring で使用されるヒストグラムメトリクスが期待どおりに処理されるために重要です。
...
次のコマンドで Collector が動作しているか確認できます:

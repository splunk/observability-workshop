...
トラベルプランナーを実行し、リクエストを送信して、マルチエージェントのトレースがSplunk Agent Observabilityに記録されたことを確認します。

{{< exercise title="アプリの実行とトレースの確認" >}}

{{< step title="環境のアクティベート" >}}
base-appディレクトリに移動し、仮想環境をアクティベートします。
...
同じ環境にサインインした2つ目のターミナルで、旅行プランニングのリクエストを送信します。コールバックが接続された状態でワークフローがエンドツーエンドで動作していることを確認するために、計画された旅程が返されます。

{{< step title="アプリへのリクエスト送信"  >}}
2つ目のターミナルで、旅行プランニングのリクエストを送信します。コールバックが接続された状態でワークフローがエンドツーエンドで動作していることを確認するために、計画された旅程が返されます。
...
Splunk Agent Observabilityコンソール（<https://console.multitenant.galileocloud.io/splunkse）で、トレースが記録されたプロジェクトとログストリームを開きます。`.env`> で `GALILEO_PROJECT` と `GALILEO_LOG_STREAM` のコメントを解除した場合は `Workshop19` / `TravelPlanner` に、そうでない場合は `default` プロジェクトと `default` ログストリームに記録されています。
{{< /step >}}

{{< step title="トレースの確認" >}}
最新のトレースを開きます。コールバックはグラフレベルで接続されているため、リクエストに対して各エージェントノードのネストされたLLM Spanを含む単一のトレースが表示されます。
...
{{< step title="Spanの検査" >}}
任意のSpanを展開し、**system and human messages**、**model response**、**model name**、**token counts**、**latency** がキャプチャされていることを確認します。

{{% notice title="トレースが表示されない場合" style="tip" icon="exclamation-triangle" %}}

* リクエストペイロードを送信する前に、別のターミナルセッションを使用して同じ環境にログインできたことを確認してください。
* アプリが実行されている環境で `GALILEO_API_KEY` が設定されていることを確認してください（`load_dotenv()` 経由で `.env` を読み込みます）。
* 正しいプロジェクトとログストリームを表示していることを確認してください。設定した場合は `GALILEO_PROJECT` / `GALILEO_LOG_STREAM` の値、設定していない場合は `default` / `default` です。
* プロジェクトが表示されない場合は、正しい権限がない可能性があります。
* `python3 main.py` を実行しているbashでSplunk Agent Observabilityのエラーがないかアプリログを確認してください。
* Webページの左上で `splunkse` 組織に所属していることを確認してください。
...
1つの旅行プランニングリクエストで、5つの別々のトレースではなく、5つのネストされたLLM Spanを含む1つのトレースが生成されます。なぜでしょうか？

{{< details summary="ここをクリックして回答を確認" >}}
LangGraphワークフロー全体がリクエストごとに1つのルートランとして実行され、コールバックがそのランのconfigに接続されているためです。LangGraphはその単一のラン内で5つのノード（coordinator、flight、hotel、activity、synthesizer）を実行するため、各ノードの `llm.invoke(...)` は同じトレース配下の子Spanになります。各ノードに新しいコールバックと新しいランを作成した場合、5つの切断されたトレース（およびセッション）が生成され、リクエストのエンドツーエンドビューが失われます。
{{< /details >}}

---
title: 4.8 分解パターン
linkTitle: 4.8 分解パターン
weight: 8
---

### シンセサイザーが示す分解パターン

最終ノードは、各専門エージェントの出力を1つの回答にまとめます。
...
これはエージェントアプリにおける典型的なパターンです。

* 作業を専門エージェントに分解する
* 中間出力を収集する
* 最終的な応答に統合する

これが、この概要から理解しておくべき主要なアーキテクチャのアイデアの1つです。

### 知識チェック

1つのエージェントに旅行プラン全体を生成させるのではなく、なぜアプリは別の `plan_synthesizer` ノードを使用しているのでしょうか？

{{< details summary="クリックして回答を表示" >}}
システムはまず問題を **専門タスク**（フライト、ホテル、アクティビティ）に分解するためです。各専門エージェントが焦点を絞った要約を生成し、`plan_synthesizer` ノードが **それらの出力を1つのまとまった旅程に統合** します。

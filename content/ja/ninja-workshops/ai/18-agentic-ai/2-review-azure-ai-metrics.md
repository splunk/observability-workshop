...
[ドキュメント](https://help.splunk.com/en/splunk-observability-cloud/observability-for-ai/splunk-ai-infrastructure-monitoring/set-up-data-integrations/cloud-services/azure-openai)に記載されている手順を使用して、Azure アカウントをワークショップ用の Splunk Observability Cloud インスタンスにすでに統合済みです。

Azure OpenAI メトリクスが含まれるように、`Cognitive Services` からメトリクスを取得する接続設定を行いました

![Azure connection](../images/AzureConnection.png)

## Azure OpenAI メトリクス

Azure OpenAI では多数のメトリクスが取得されます
...
Splunk Observability Cloud は、OpenTelemetry の生成 AI クライアントおよびモデルサーバーメトリクスを収集し、Azure で動作する Open AI 大規模言語モデル（LLM）サービスのトークン使用量を追跡します。
...
Splunk Observability Cloud は Azure OpenAI 用の組み込みダッシュボードを提供しており、以下の項目をすぐに確認できます

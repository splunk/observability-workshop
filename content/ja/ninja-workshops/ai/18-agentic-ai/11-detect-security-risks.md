...
> どこを変更すべきか分からない場合やアプリケーションが動作しなくなった場合は、
...
これは、LLM がレスポンスの一部としてユーザーのクレジットカード番号を含めたシナリオを効果的にシミュレートします。これは明らかなセキュリティリスクであり、PCI 違反です。
...
> `localhost:9999/agentic-ai-app:app-with-security-risk` の代わりに
...
## Cisco AI Defense でのイベント表示

ワークショップ参加者は AI Defense アプリケーションに直接ログインすることはできません。
しかし、AI Defense ダッシュボードを表示できた場合、このリクエストに対してイベントがログに記録され、プロンプトに含まれるクレジットカード番号が自動的にマスクされていることが確認できます。

![AI Defense Events](../images/AIDefenseEvents.png)

AI Defense ではポリシーを設定して、特定の種類のセキュリティ問題を監視するかブロックするかを指定できます。今回のケースでは、PCI 関連の問題を監視のみするように設定しています。

## Splunk Observability Cloud でのデータ表示

Splunk Observability Cloud に戻って、トレースがどのように表示されるか確認しましょう。

`APM` に移動し、`AI agents` を選択します。環境名が選択されていることを確認してください（例: `agentic-ai-$INSTANCE`）。ページにセキュリティリスクが表示されていることに気付くでしょう！
...
`activity_specialist` エージェントの `invoke_agent` スパンを見ると、LLM がレスポンスで顧客のクレジットカード番号を平文で公開したことにより、PCI セキュリティリスクが検出されブロックされたことが分かります:

![Trace With Security Risk](../images/TraceWithSecurityRisk.png)

セキュリティリスクをクリックすると、追加の詳細と Cisco AI Defense でイベントを表示するためのリンクが表示されます:

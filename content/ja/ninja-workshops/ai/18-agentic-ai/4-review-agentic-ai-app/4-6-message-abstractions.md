...
LangChain と LangGraph では、メッセージは通常、誰が話しているか、会話の中でどのような役割を果たしているかによって分類されます

* **System message**: AI の動作に関するルールとコンテキストを設定します。モデルがインタラクション全体を通じてどのように応答すべきかを導く、指示、制約、トーン、目標を定義します。
* **Human message**: ユーザーからの入力です。AI が応答すべき質問、リクエスト、または情報が含まれます。
* **AI message**: モデルの応答です。システムの指示と人間の入力に基づいて生成されたアシスタントの出力を表します。
{{< /details >}}

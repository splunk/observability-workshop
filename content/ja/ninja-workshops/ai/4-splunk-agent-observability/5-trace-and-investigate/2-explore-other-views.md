...
Splunk Agent Observabilityのプロジェクトと `default` エージェントストリームに戻ります。

{{< /step >}}

{{< step title="Agent Graphを表示する" >}}

**`Agent graph`** タブをクリックします。このタブはエージェントストリーム内のすべてのTraceにわたるエージェントの動作を集約して表示します。最も一般的な実行パス、レイテンシーと頻度のパターンが表示されるため、実際のユーザーに対してエージェントがどのように動作しているかを素早く確認できます。
...
個々のTraceを読むことは単一のインシデントには有効ですが、何百万ものTraceを手動で検査することはできません。次の章では、すべてのTraceを自動的にスコアリングする **evaluators** を有効にし、根拠のない医療アドバイスなどの問題が自動的に浮上するようにします。

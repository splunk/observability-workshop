...
Jenkinsベースのスマートエージェントデプロイシステムは、ハブアンドスポークアーキテクチャを採用しています。AWS VPC内のJenkinsエージェントがSSH経由で複数のターゲットホストへのデプロイを管理します。
...
すべてのインフラストラクチャは、共有セキュリティグループを持つ単一のAWS VPC内で動作します。JenkinsエージェントはプライベートIPを介してターゲットホストと通信するため、ターゲットホストにパブリックIPアドレスは不要です。
...
システムはあらゆる規模のデプロイをサポートするために自動バッチ処理を使用します。デフォルトでは、ホストは256台ずつのバッチで処理され、バッチ内のすべてのホストは並列でデプロイされます。
...
アーキテクチャを理解できたので、次はJenkinsのセットアップと認証情報の設定に進みます。

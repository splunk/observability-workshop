...
time: 2 minutes
weight: 1
description: この演習では、Splunk AppDynamicsコントローラーにアクセスし、アプリケーションへのトランザクション負荷を確認し、必要に応じてアプリケーションとトランザクション負荷を再起動します。
draft: true
---
...
注意: Calls列が表示されていない場合は、View Optionsツールバーボタンをクリックしてその列を表示できます。
...
2. Grid Viewをクリックします。
3. 各NodeのApp Agent Statusが過去1時間で90%を超えていることを確認します。

![Verify App Load](images/verify-app-load-03.png)

## 必要に応じてアプリケーションとトランザクション負荷を再起動

前のステップで実行したチェックのいずれかが確認できなかった場合は、アプリケーションVMにSSHで接続し、以下の手順に従ってアプリケーションとトランザクション負荷を再起動します。EC2インスタンスのIPアドレス、インスタンスにSSH接続するために必要なユーザー名とパスワードを準備してください。ローカルマシンでターミナルを開き、以下を入力します。
...
まだ実行中のアプリケーションJVMが残っている場合は、以下のコマンドを使用して残りのJVMを終了します。
...
以下の画像のように、ポート8080がApache Tomcatによって使用されていることを示す出力が表示されます。

...
- 2番目のホストはラボで使用する Supercar Trader アプリケーションを実行しています。ここに AppDynamics エージェントをインストールするホストであり、以降は Application VM と呼びます。
...
Supercar-Trader コレクションの目的は、AppDynamics Controller のための動的トラフィック（Business Transactions）を生成することです。

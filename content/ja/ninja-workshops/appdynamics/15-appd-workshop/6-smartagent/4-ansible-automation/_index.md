...
このデプロイアーキテクチャでは、Ansible Control Node を活用してターゲットホストへの Smart Agent のインストールと設定をオーケストレーションします。
...
* **Ansible Control Node**: Playbook を実行するマシンです（例: ラップトップやジャンプホスト）。
...
3. **インストールの確認**:
    ...
    インストールされた Ansible のバージョンが表示されます。

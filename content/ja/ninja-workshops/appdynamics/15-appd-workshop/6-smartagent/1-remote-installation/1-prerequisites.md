...
- **SSH アクセス**: Smart Agent をインストールするすべてのリモートホストへの SSH アクセスが必要です
- **SSH 秘密鍵**: 認証用に設定された SSH 秘密鍵が必要です
- **Sudo 権限**: コントロールノードのユーザーが smartagentctl を実行するための sudo 権限が必要です
...
- **Controller Port**: 通常、HTTPS 接続では 443 です
...
- **SSH サーバー**: SSH デーモンが実行中で、接続を受け付けている状態
...
リモートホストが相互に通信でき、インターネットに接続できることを確認します

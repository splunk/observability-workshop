...
{{% notice style="danger" %}}
「Cleanup All Agents」ワークフローは `/opt/appdynamics` を完全に削除します。この操作は元に戻すことができません。十分注意して使用してください！
{{% /notice %}}

## ワークフローの構造

すべてのバッチ処理ワークフローは、一貫した 2 ジョブ構造に従います:
...
**目的**: 各バッチに対して並列に実行し、バッチ内のすべてのホストで特定の操作を実行します
...
3. **バッチは順次処理されます** - ランナーへの過負荷を避けるためです
...
1. **Stop and Clean Smart Agent** - サービスを停止しデータをクリーン
2. **Deploy Smart Agent** - 更新されたバージョンで再デプロイ

...
GitHub Actions ベースの Smart Agent デプロイメントシステムは、AWS VPC 内のセルフホストランナーを使用して、SSH 経由で複数のターゲットホストへのデプロイメントをオーケストレーションします。
...
すべてのインフラストラクチャは、共有セキュリティグループを持つ単一の AWS VPC 内で実行されます。セルフホストランナーはプライベート IP を介してターゲットホストと通信します。
...
このシステムには、4 つのカテゴリに分類された 11 のワークフローが含まれています:
...
- セルフホストランナーの過負荷を防止します
- 各バッチは 256 の並列 SSH 接続を開きます
- シーケンシャル処理により安定したパフォーマンスを確保します

**設定可能:**

- デフォルトバッチサイズ: 256 (GitHub Actions マトリクスの上限)
- より小さなバッチにするためにワークフロー入力で調整可能
- 速度とリソース使用量のバランス
...
アーキテクチャを理解できたところで、GitHub のセットアップとセルフホストランナーの設定に進みましょう。

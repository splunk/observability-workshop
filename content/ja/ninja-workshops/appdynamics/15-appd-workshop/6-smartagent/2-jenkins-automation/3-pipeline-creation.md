...
GitHub リポジトリには、Smart Agent のライフサイクルを管理するための4つの主要なパイプラインが含まれています

1. **Deploy Smart Agent** - Smart Agent サービスをインストールして起動します
2. **Install Machine Agent** - smartagentctl を使用して Machine Agent をインストールします
3. **Install Database Agent** - smartagentctl を使用して Database Agent をインストールします
...
### 方法 2: パイプラインスクリプトの直接入力

別の方法として、Jenkinsfile の内容を Jenkins に直接コピーすることもできます。

1. **Create New Item**（方法 1 と同様）
2. **Pipeline** セクションで
...
{{% notice style="tip" %}}
方法 1（SCM）が推奨されます。パイプラインをバージョン管理に保持し、更新が容易になるためです。
{{% /notice %}}

## パイプラインパラメータ

各パイプラインは設定可能なパラメータを受け付けます。以下は、メインのデプロイメントパイプラインの主要なパラメータです
...
Cleanup パイプラインには、誤って削除することを防ぐための確認パラメータが含まれています。クリーンアップを実行するには `CONFIRM_CLEANUP` にチェックを入れる必要があります。
{{% /notice %}}

## パイプライン構造の理解

デプロイメントパイプラインの主要なコンポーネントを確認しましょう
...
パイプラインの完了後に、成功・失敗に関わらず実行されるアクションを定義します。

## パイプラインの命名規則

分かりやすさと整理のために、一貫した命名規則を使用します
...
数字のプレフィックスにより、Jenkins ダッシュボードで論理的な順序が維持されます。
...
1. 1つの IP のみを含むテスト用クレデンシャル `deployment-hosts-test` を作成します
2. パイプラインを一時的に変更して、このクレデンシャルを使用します
3. パイプラインを実行し、単一ホストで動作することを確認します
4. 確認後、完全なホストリストを使用するように更新します

### 2. 構文チェック

Jenkins には組み込みの構文バリデータが用意されています

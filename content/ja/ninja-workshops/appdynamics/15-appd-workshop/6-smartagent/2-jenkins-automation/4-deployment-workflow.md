...
パイプラインの設定が完了したので、最初の Smart Agent デプロイメントを実行する手順を説明します。
...
**Build** をクリックした後、以下が表示されます:

1. **Build added to queue** - Build History にビルド番号が表示されます
2. **ビルド番号をクリック**（例: #1）して詳細を確認します
3. **Console Output をクリック** してリアルタイムログを確認します
...
5. ✅ **Verify Installation** - Smart Agent ステータスの確認
...
2. **Build with Parameters** をクリックします
3. `CONFIRM_CLEANUP` チェックボックスを**オン**にします
4. **Build** をクリックします
...
**原因**: `deployment-hosts` クレデンシャルが存在しないか、正しくありません
...
次に、大規模デプロイメントのスケーリングと運用に関する考慮事項を説明します。

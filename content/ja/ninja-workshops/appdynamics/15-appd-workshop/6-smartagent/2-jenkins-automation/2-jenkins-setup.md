...
Jenkins エージェントはプライベート IP 経由でターゲット EC2 インスタンスに到達できる必要があります。主に2つのオプションがあります

### オプション A: EC2 インスタンスをエージェントとして使用

1. **ターゲットホストと同じ VPC に EC2 インスタンスを起動します**

2. **Java をインストールします**（Jenkins に必要）
...
このワークショップのすべてのパイプラインは `linux` ラベルを使用します。エージェントがこのラベルで設定されていることを確認してください。
{{% /notice %}}
...
以下に移動します**Manage Jenkins → Credentials → System → Global credentials (unrestricted)**

パイプラインが動作するために、3つの認証情報を作成する必要があります。
...
- **ID**: `ssh-private-key`（正確に一致する必要があります）
...
- **ID**: `deployment-hosts`（正確に一致する必要があります）
...
- **ID**: `account-access-key`（正確に一致する必要があります）
...
Smart Agent の ZIP ファイルは、Jenkins がアクセスできる場所に配置する必要があります。推奨されるアプローチは、Jenkins のホームディレクトリに保存することです。
...
パイプラインは Smart Agent の ZIP を次のパスで参照します`/var/jenkins_home/smartagent/appdsmartagent.zip`

以下のいずれかを行うことができます

1. この正確なパスに ZIP を配置する
2. パイプラインパラメータ `SMARTAGENT_ZIP_PATH` を変更して ZIP の場所を指定する

## 設定の確認

パイプラインの作成に進む前に、セットアップを確認してください
...
SSH が動作することを確認するため、シンプルなテストパイプラインを作成します
...
Jenkins の認証情報とエージェントの設定が完了しました。デプロイメントパイプラインの作成に進みましょう！

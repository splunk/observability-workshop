...
3. **新ワークショップ検出**: 翻訳対象ファイルから新しいワークショップディレクトリを検出（後述）
...
**新ワークショップの判定基準**:
- `content/en/{category}/{workshop-name}/` レベルのディレクトリが前回翻訳タグ時点で存在しなかった場合、そのワークショップは「新規」と判定
- `git rev-parse --verify` で前回タグ時点のディレクトリ存在を確認

**動作**:
- 新ワークショップ検出時: `--draft` フラグ付きでPRを作成、PR本文に検出されたワークショップ一覧を記載
- 新ワークショップなし: 通常のPRを作成（既存動作）
...
翻訳が完了すると、upstreamリポジトリにPRが自動的に作成されます。新しいワークショップが検出された場合はドラフトPRとして作成され、人間のレビューが必要です。レビュー時は以下を確認してください：

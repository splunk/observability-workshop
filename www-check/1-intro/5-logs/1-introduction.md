...
This view will include **all** the log lines from **all** services that participated in the back-end transaction started by the end-user interaction with the Astronomy Shop.

Even in a small application such as our Astronomy Shop, the sheer amount of logs found can make it hard to see the specific log lines that matter to the actual incident we are investigating.
...
* We did see that this error from the **paymentservice** `v350.10` caused multiple retries and a long delay in the response back from the Astronomy Shop checkout.
